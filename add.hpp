...
int add_new(vector<string> &newf) {
    for (int i = 0; i < newf.size(); i++) {
        ...
        // copy file from current version.
        string s = cwd;
        ...
        // copy new files to the base folder.
        ifstream fin;
        ofstream fout;
    ...
    string main_indes_path = string(cwd) + "/.mygit/main_index.txt";

    ofstream main_indes_file(main_indes_path, ios::app);
    for (int i = 0; i < newf.size(); i++) {
        string filename = newf[i];
        string s = cwd;
        string path_in = s + "/" + filename;
        string path_out = base_path + "/" + filename;

        // in main index file add the new file and the version no in which it came into existence.
        main_indes_file << filename << " " << version_no << "\n";
...
int add() {
    ...
    // goto current version index file and create an map.
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (map_index.find(files[i]) == map_index.end()) // new file.
        {
            new_files.push_back(files[i]);
        } 
        else if (map_index[files[i]] != sha) // modified file.
        {
    ...
    if (new_files.size() != 0) {
        ...
        if (new_processed) new_files.clear();
        if (new_files.size()) {
            cout << "new_files clear" << endl;
        }
    }
    ...
    if (deleted_files.size() != 0) {
        ...
        if (deleted_processed) deleted_files.clear();
    }

    if (new_files.size() == 0 and deleted_files.size() == 0 and modified_files.size() == 0) {
        return 1;
    } else {
        return 0;
    }
